...
# -------------------------------------------------------------------------------#
# centernet的非极大抑制在解码之前进行
# 非极大抑制  使用最大池化的方法完成topk的操作，利用3x3的池化核在热力图上搜索，然后只保留一定区域内得分最大的框
# 在实际使用时发现，当目标为小目标时，确实可以不在解码之后进行非极大抑制的后处理，如果目标为大目标，网络无法正确判断目标的中心时，
# 还是需要进行非极大抑制的后处理的
# -------------------------------------------------------------------------------#
...
def topk(hm, max_objects=100):
    ...
    #-------------------------------------------#
    #   将所有结果平铺，获得(b, 128 * 128 * 80)       (b, h * w * c)
    #-------------------------------------------#
